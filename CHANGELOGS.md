...
* **14% minimum entry price floor** — signals are now rejected when `yes_price < 0.14` at evaluation time. Based on 48-trade segmented analysis: sub-14% entries had 29% win rate and -$12.23 PnL; 18-25% entries had 47% win rate and +$10.70 PnL. Raised from previous 10% floor. Data-driven, not arbitrary.
* **SL alert shows capped exit price** — Stop Loss alerts previously showed `Now: 9.0%` (the noisy poll price at check time). Now shows `Exit: 12.6%` — the actual simulated exit price where the stop order would have filled. Consistent with how PnL is calculated (always capped at threshold, not poll price). Both DB and CSV paths fixed.
* **Position buttons numbered sequentially** — inline keyboard buttons on `/pos` previously showed the database row ID (`#38 Seattle`, `#39 Buenos Air`). Now shows position number within the current open list (`#1 Seattle`, `#2 Buenos Air`). Continues across pages (page 2 starts at #9). DB row ID unchanged internally for navigation.
* **`/drift` command bug fixed** — `/drift` was calling `refresh_forecasts()` which updates `last_model_cache` before comparing, making the delta always zero. Fix: snapshot the cache before fetching, then compare snapshot vs updated values manually. Auto drift alerts were unaffected (correct logic already in place).
* **Drift alert format overhauled** — auto drift alerts were sending one separate message per city. Now sends a single combined message with timestamp at top: `📊 GFS Drift · 04:40 UTC` followed by compact lines `📈 Miami +3.0°C (25.0→28.0)`. `/drift` command follows same format with no per-line timestamps.
* **Spread filter city name fixed** — Railway logs were showing `Spread check: ? ask 22¢ bid 19¢` with `?` as the city name. Fixed to use `city_key` from bucket dict correctly. Display only — no functional impact.
* **PostgreSQL now live** — migrated from CSV fallback to persistent Railway PostgreSQL. Trade history survives container restarts, redeployments, and crashes. CSV fallback still active for zero-DATABASE_URL environments.
