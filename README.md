...
[![v3.1](https://img.shields.io/badge/version-v3.1-orange.svg)]()
...
## Notable v3.1 Additions

- Strengthened the live operator workflow around real exchange truth:
  `/orders` now centers on exchange open orders, `/pos` can recover exchange-backed live positions, and wallet/home/pnl summaries align more closely with the real live book.
- Added richer live controls and risk settings:
  `Automation` auto/manual mode, `Max Positions / Day`, and deep-value half-size defaults for both Kelly and flat sizing.
- Improved live bookkeeping and exports:
  cleaner execution exports, better recent-trade formatting, pending review surfacing, and local Polymarket history backfill for missing created/opened timestamps.
- Cleaned up live Telegram presentation:
  cents-based prices, compact `/pos` cards, cleaner `/pnl` and `/orders` wording, gear emoji on settings, and safer close/exit flows.
- Improved live position recovery and reconciliation:
  exchange-backed live position fallback, stricter stale-row cleanup, and better TP adoption when the exchange already has the order resting.
